...
description: Build interactive components with purposeful motion. Light discovery, then generate against the cookbook.
---
...
1. `references/cookbook.md` — the recipe source for all motion code
2. `references/principles/reduced-motion.md` — `prefers-reduced-motion` is mandatory in everything you generate
3. `references/creation-gotchas.md` — failure modes when writing motion; self-check against these
...
- **Read `references/perspectives/emil-kowalski.md`** if Emil is primary/secondary — restraint, the frequency rule, when NOT to animate
- **Read `references/perspectives/jakub-krehel.md`** if Jakub is primary/secondary — production polish judgment, subtlety bar
- **Read `references/perspectives/jhey-tompkins.md`** if Jhey is primary/secondary — playful expression, what motion could become

The designer files give you the **judgment** (should this animate, what feel). The cookbook gives you the **code**.

If the component involves complex or numerous animations, also read `references/guidelines/performance.md`.
